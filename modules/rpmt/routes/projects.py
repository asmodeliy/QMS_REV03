...
@router .post ("/tasks/{tid}/update",response_class =HTMLResponse )
def task_update (
request :Request ,
tid :int ,
...
@router .post ("/tasks/{tid}/update-json")
...
<html>
<head><meta charset="utf-8"><title>파일 미리보기 오류</title></head>
<body style="padding:40px;font-family:sans-serif;">
    <h2>파일 미리보기 생성 실패</h2>
    <p>파일을 처리하는 중에 오류가 발생했습니다.</p>
...
def get_my_tasks(request: Request, db: Session = Depends(get_db)):
    ...
    user_dept = request.session.get('department', 'QM')  # 기본값 설정
