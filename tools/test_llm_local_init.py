...
# ensure RPMT or top-level services.py is importable
from pathlib import Path
ROOT = Path(__file__).resolve().parent.parent
# Support both layouts: project root (top-level services.py) and RPMT package
sys.path.insert(0, str(ROOT))
sys.path.insert(0, str(ROOT / 'RPMT'))

# Import services robustly: prefer top-level services, then RPMT.services
try:
    try:
        import services  # top-level services.py
    except ModuleNotFoundError:
        from RPMT import services  # package layout
    llm_service = getattr(services, 'llm_service', None)
    if llm_service is None:
        raise ImportError('services.llm_service is not available')
